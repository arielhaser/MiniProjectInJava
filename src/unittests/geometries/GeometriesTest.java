...
public class GeometriesTest {
    ...
    public void findIntersections() {
        ...
        Sphere sp = new Sphere(2, new Point3D(0,-2,2));
        Plane pl = new Plane(new Point3D(1,2,4),new Point3D(4,5,7),new Point3D(8,8,9));
        ...
        List<Point3D> result = ge.findIntersections(new Ray(new Point3D(-3,0,0),
        ...
        //EC03: not single shape has inserted (0 points)
        result = ge.findIntersections(new Ray(new Point3D(0,3,0),
                new Vector(-1.5511254045, -3.4504801707, 1.6501650632)));
        assertEquals("Ray's line out of shape", null, result);

        //EC04: one shape inserted (plane == 1 points)
        result = ge.findIntersections(new Ray(new Point3D(0,3,0),
                new Vector(3,3,3)));
        assertEquals("Wrong number of points", 1, result.size());

        //EC05: all shapes inserted (4 points)
        result = ge.findIntersections(new Ray(new Point3D(-3,0,0),
                new Vector(6.4595459051, -1.783079407, 3.2426583979)));
